...
namespace Oragon.Media.Http
{
    ...
    public abstract class HttpContent : BaseDisposable
    {
    ...
    {
        public readonly MemorySegment Data;

        public BinaryContent(byte[] data, string name = null) : this(new MemorySegment(data), name) { }
        
        public BinaryContent(MemorySegment data, string name = null) : base(name)
        {
    ...
    public class MultipartContent : HttpContent, IEnumerable<HttpContent>
    {
        ...
        public MultipartContent(byte[] boundary) : this(new MemorySegment(boundary)) { }

        public MultipartContent(MemorySegment boundary)
        {
        ...
        public readonly MemorySegment Boundary;

        public readonly List<HttpContent> Contents = new List<HttpContent>();

        public void Add(HttpContent content)
        {
            if (IDisposedExtensions.IsNullOrDisposed(content)) throw new ArgumentException("content cannot be null or disposed");
