...
namespace Oragon.Media.Http
{
    ...
    public class HttpClient : SuppressedFinalizerDisposable, ISocketReference
    {
        ...
        internal static void ConfigureHttpSocket(Socket socket)
        {
            ...
            {
                //
                //SocketExtensions.EnableTcpNoSynRetries(socket);

                //
                //SocketExtensions.EnableTcpTimestamp(socket);

                //
                //SocketExtensions.SetTcpOffloadPreference(socket);
                ...
                if(OperatingSystemExtensions.IsWindows) SocketExtensions.DisableTcpRetransmissions(socket);
        ...
        int m_SentBytes, m_ReceivedBytes,
             ...
             m_ReceivedMessages,
             m_PushedMessages,
             m_ResponseTimeoutInterval = (int)TimeSpanExtensions.MicrosecondsPerMillisecond;
        ...
        public HttpClient(Uri location, int bufferSize = DefaultBufferSize, Socket existing = null, bool leaveOpen = false, int responseTimeoutInterval = (int)TimeSpanExtensions.MicrosecondsPerMillisecond, bool shouldDispose = true)
            :base(shouldDispose)
        {
        ...
        public HttpMessage SendHttpMessage(HttpMessage message, out SocketError error, bool useClientProtocolVersion = true, bool hasResponse = true, int attempts = 0)
        {
            ...
            unchecked
            {
                ...
                {
                    ILoggingExtensions.Log(Logger, ToString() + "@SendHttpMessage: " + ex.Message);
                }
                finally
                {
