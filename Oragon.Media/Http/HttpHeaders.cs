...
namespace Oragon.Media.Http
{
    ...
    public class HttpHeaders
    {
        ...
        public const char HyphenSign = (char)ASCII.HyphenSign, SemiColon = (char)ASCII.SemiColon, Comma = (char)ASCII.Comma;

        internal protected static string[] TimeSplit = new string[] { HyphenSign.ToString(), SemiColon.ToString() };

        internal protected static char[] SpaceSplit = new char[] { (char)ASCII.Space, Comma };

        internal protected static char[] ValueSplit = new char[] { (char)ASCII.EqualsSign, SemiColon };
        ...
        {
            return string.Concat(string.Concat(type, (typeSeperator ?? (char)ASCII.EqualsSign).ToString()), string.Join((valueSeperator ?? (char)ASCII.HyphenSign).ToString(), startPart, endPart));
        }
                ...
                case System.Net.AuthenticationSchemes.Basic:
                    {
                        ...
                        //result = string.Concat(Http.HeaderFields.Authorization.Basic, new string((char)ASCII.Space, 1), Convert.ToBase64String(encoding.GetBytes(credential.UserName + ':' + credential.Password)));

                        result = string.Concat("Basic", new string((char)ASCII.Space, 1), Convert.ToBase64String(encoding.GetBytes(credential.UserName + ':' + credential.Password)));
                ...
                case System.Net.AuthenticationSchemes.Digest:
                    {
                        ...
                        string usernamePart = credential.UserName,
                           ...
                           uriPart = location != null && location.IsAbsoluteUri ? location.AbsoluteUri : new String((char)ASCII.BackSlash, 1);
        ...
        public static string[] SplitSpace(string input) { return input.Split((char)ASCII.Space); }

        public static string[] SplitComma(string input) { return input.Split(SemiColon); }

        public static string[] SplitEqual(string input) { return input.Split((char)ASCII.EqualsSign); }
        ...
        public static ConcurrentThesaurus<string, string> ParseHeader(string input)
        {

            //Todo, instead of split use substring...

            ConcurrentThesaurus<string, string> result = new ConcurrentThesaurus<string, string>();
