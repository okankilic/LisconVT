...
namespace Oragon.Media.Http
{
    ...
    /// </summary>
    public class HttpMessage : SuppressedFinalizerDisposable, IPacket
    {
        #region Statics
        ...
        internal protected char[] m_EncodedLineEnds = UTF8.LineEndingCharacters,
            ...
            m_EncodedComma = UTF8.CommaCharacters;
        ...
        public HttpMethod HttpMethod
        {
            ...
            get
            {
                ...
                if (false.Equals(string.IsNullOrWhiteSpace(MethodString))) System.Enum.TryParse<HttpMethod>(MethodString, true, out parsed);
        ...
        public Encoding HeaderEncoding
        {
            ...
            set
            {
                ...
                m_EncodedComma = m_HeaderEncoding.GetChars(UTF8.CommaBytes);
            ...
            : base(shouldDispose)
        {
            ...
            if (object.ReferenceEquals(data, null) || offset < 0 || offset >= (length = Binary.Clamp(length, 0, data.Length)))
            {
                return;
            }
        ...
        Parse:
            ...
            if ((sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer.GetBuffer(), m_EncodedLineEnds, m_Buffer.Position, m_Buffer.Length, out StatusLine, out read, true)).Equals(false) &&
            ...
            else if (MessageType == HttpMessageType.Invalid)
            {
                ...
                if ((index = parts[0].IndexOf((char)ASCII.ForwardSlash)) > 0)
                {
                ...
                else if (partsLength > 2 && (index = parts[2].IndexOf((char)ASCII.ForwardSlash)) > 0)
                {
        ...
        virtual protected bool ParseHeaders(bool force = false)
        {
            try
            {
                        ...
                        (remains = max - position) > 0)
                {
                    ...
                    sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, buffer, m_EncodedLineEnds, position, remains, out rawLine, out justRead, true);
        ...
        internal protected virtual bool ParseContentLength(bool force = false)
        {
            ...
            if (false.Equals(int.TryParse(ASCII.ExtractNumber(contentLength), System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out m_ContentLength)))
            {
        ...
        internal protected virtual Encoding ParseContentEncoding(bool force = false, bool raiseWhenNotFound = false)
        {
            ...
            if (false.Equals(string.IsNullOrWhiteSpace(contentEncoding)))
            {
                ...
                else if (true.Equals(raiseWhenNotFound)) TaggedExceptionExtensions.RaiseTaggedException(contentEncoding, "The given message was encoded in a Encoding which is not present on this system and no fallback encoding was acceptible to decode the message. The tag has been set the value of the requested encoding");
            ...
            if (available > 0)
            {
                ...
                else //Content-Length style message
                {
                    if (existingBodySize == 0)
                        m_Body = decoder.GetString(buffer, position, Binary.Min(ref available, ref remaining));
        ...
        {
            if (IsDisposed && IsPersistent.Equals(false)) return MemorySegment.EmptyBytes;
        ...
        public virtual int CompleteFrom(System.Net.Sockets.Socket socket, MemorySegment buffer)
        {
            ...
            if (false.Equals(hasSocket))
            {
                ...
                if (false.Equals(IDisposedExtensions.IsNullOrDisposed(buffer)) && buffer.Count > 0)
                {
                    //Write the new data
        ...
        //Top:
            ...
            //If it was not present then do not parse further
            if ((sawDelemit = EncodingExtensions.ReadDelimitedDataFrom(HeaderEncoding, m_Buffer, m_EncodedLineEnds, length, out ChunkLine, out read, true)).Equals(false))
            {
                return -1;
            }
...
namespace Media.UnitTests
{
    ...
    internal class HttpMessgeUnitTests
    {
        public void TestRequestsSerializationAndDeserialization()
        {
            ...
            foreach (HttpMethod method in System.Enum.GetValues(typeof(HttpMethod)))
            {
                using (HttpMessage request = new HttpMessage(HttpMessageType.Request))
                {
                    ...
                    using (HttpMessage serialized = new HttpMessage(bytes))
                    {
                    ...
                    request.Location = HttpMessage.Wildcard;
        ...
        public void TestResponsesSerializationAndDeserialization()
        {
            ...
            foreach (HttpStatusCode statusCode in System.Enum.GetValues(typeof(HttpStatusCode)))
            {
                using (HttpMessage response = new HttpMessage(HttpMessageType.Response)
                {
                    Version = 7,
        ...
        public void TestHeaderSerializationAndDeserialization()
        {
            ...
            {
                Version = 7,
                HttpStatusCode = (HttpStatusCode)7
            })
            {
        ...
        public void TestMessageSerializationAndDeserializationFromString()
        {
            ...
            using (HttpMessage message = HttpMessage.FromString(TestMessage))
            {
                string output = message.ToString();

                if (message.MessageType != HttpMessageType.Request ||
                               message.HttpMethod != HttpMethod.GET ||
                               message.Version != 1.1 ||
            ...
            using (HttpMessage message = HttpMessage.FromString(TestMessage))
            {
                ...
                if (message.MessageType != HttpMessageType.Response ||
                    message.Version != 1.0 ||
                    message.HttpStatusCode != HttpStatusCode.OptionNotSupported ||
            ...
            using (HttpMessage response = HttpMessage.FromString(TestMessage))
            {
                string output = response.ToString();

                if (response.MessageType != HttpMessageType.Response ||
                    response.Version != 1.0 ||
                    response.HttpStatusCode != HttpStatusCode.OptionNotSupported ||
        ...
        //{
        //    using (HttpMessage message = new HttpMessage(HttpMessageType.Response))
        //    {
        //        message.HttpStatusCode = HttpStatusCode.OK;
        ...
        //            using (HttpMessage toComplete = new Http.HttpMessage(MemorySegment.EmptyBytes))
        //            {
        ...
        //                    using (MemorySegment chunkData = new MemorySegment(buffer, offset, chunkSize))
        //                    {
        ...
        //{
        //    using (HttpMessage message = new HttpMessage(HttpMessageType.Response, 1.0, HttpMessage.DefaultEncoding)
        //    {
