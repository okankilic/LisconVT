...
namespace Oragon.Media.Cryptography
{
    ...
    public sealed class MD5
    {
        ...
        internal static uint[] Converter(byte[] input, int ibStart)
        {
            ...
            bool isBigEndian = Binary.IsBigEndian;
            ...
            else for (int i = 0; i < BitsInWord; i++) //Read 16 uint values (use length for custom hash)
            {
                ...
                result[i] = Binary.ReadU32(input, ref ibStart, isBigEndian);
            }
        ...
        internal static byte[] GetHashFinalBlock(byte[] input, int ibStart, int cbSize, ref ABCDStruct ABCD, Int64 len)
        {
            ...
            Binary.Write64(length, Zero, isBigEndian, len);
            ...
            //Write in Little Endian
            Binary.Write32(output, Zero, isBigEndian, ABCD.A);
            Binary.Write32(output, BitsInNibble, isBigEndian, ABCD.B);
            Binary.Write32(output, BitsInByte, isBigEndian, ABCD.C);
            Binary.Write32(output, 12, isBigEndian, ABCD.D);
